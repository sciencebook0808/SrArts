      ...
      >
        <div className="p-5 border-b border-border">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <Quote className="w-5 h-5 text-primary" /> Repost with your thoughts
          </h3>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div className="border border-border rounded-xl p-3 bg-accent-subtle/30 text-sm">
        ...
        <div className="p-5 border-t border-border flex gap-3 justify-end">
          <button onClick={onCancel} className="px-4 py-2 text-sm rounded-xl border border-border hover:bg-accent-subtle transition-colors">
            Cancel
          </button>
          <button
...
export function PostCard({ post, currentUserId, onDeleted }: PostCardProps) {
  ...
  useState(() => { void fetchLikeState(); });

  const toggleLike = async () => {
    if (!isSignedIn) { setAuthPrompt('like'); setTimeout(() => setAuthPrompt(''), 3000); return; }
    const willLike = !liked;
    setLiked(willLike); setLikeCount(c => c + (willLike ? 1 : -1));
    try {
      ...
      if (res.ok) {
        ...
        if (typeof d.count  === 'number')  setLikeCount(d.count);
        if (typeof d.liked  === 'boolean') setLiked(d.liked);
      } else { setLiked(!willLike); setLikeCount(c => c + (willLike ? -1 : 1)); }
    } catch { setLiked(!willLike); setLikeCount(c => c + (willLike ? -1 : 1)); }
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between p-4 pb-3">
          <div className="flex items-center gap-3">
            {post.authorImage ? (
              ...
              <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                <span className="text-primary font-bold">{post.authorName.charAt(0).toUpperCase()}</span>
              </div>
            )}
            <div>
          ...
          {isOwner && (
            <div className="relative">
              <button onClick={() => setShowMenu(v => !v)} className="p-1.5 rounded-lg hover:bg-accent-subtle text-muted-foreground transition-colors">
        ...
        {/* Attached image */}
        {post.imageUrl && (
          <div className="relative w-full aspect-video bg-accent-subtle mx-0">
            <Image src={post.imageUrl} alt="Post image" fill className="object-cover" sizes="(max-width: 640px) 100vw, 600px" />
          </div>
        )}
          ...
          <div className="mx-4 mb-3 border border-border rounded-xl p-3 bg-accent-subtle/30">
            <div className="flex items-center gap-2 mb-1.5">
              {post.repostOf.authorImage && (
                <div className="w-5 h-5 rounded-full overflow-hidden shrink-0">
                  <Image src={post.repostOf.authorImage} alt={post.repostOf.authorName} width={20} height={20} className="object-cover" />
                </div>
              )}
            ...
            {post.repostOf.imageUrl && (
              <div className="relative w-full aspect-video rounded-lg overflow-hidden mt-2 bg-accent-subtle">
                <Image src={post.repostOf.imageUrl} alt="" fill className="object-cover" sizes="300px" />
              </div>
            )}
          </div>
        )}
        ...
        <div className="px-4 py-3 border-t border-border flex items-center gap-1">
          ...
          <div className="relative">
            ...
            <AnimatePresence>
              {authPrompt === 'like' && (
                <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute bottom-full mb-2 left-0 bg-white border border-border rounded-xl shadow-lg px-3 py-2 text-xs whitespace-nowrap z-10">

 ...
 * Social cards now use the enriched SocialProfileCard component
 * which shows: avatar, name, bio, followers/following/posts, category, external link, follow button.
 *
 * Data is ALWAYS read from DB — never calls social APIs directly.
 */
...
import { SocialProfileCardsGrid } from '@/components/social/social-profile-card';
...
import type { SocialProfileCardData } from '@/components/social/social-profile-card';
...
export default async function AboutPage() {
  ...
  // Map DB accounts → SocialProfileCardData (type-safe projection)
  const profileCards: SocialProfileCardData[] = socialAccounts.map(a => ({
    id:              a.id,
    platform:        a.platform,
    username:        a.username,
      ...
      <main className="w-full min-h-screen bg-[#f4f2ef] overflow-x-hidden">
        ...
        <div className="pt-14 md:pt-0">
          ...
          <div
            ...
            style={{ background: 'linear-gradient(135deg, oklch(0.40 0.17 150) 0%, oklch(0.55 0.19 155) 50%, oklch(0.45 0.15 145) 100%)' }}
          >
            {bannerImg && (
              <Image src={bannerImg} alt="Studio banner" fill className="object-cover opacity-70" priority sizes="100vw" />
            )}
            <svg className="absolute inset-0 w-full h-full opacity-20" viewBox="0 0 800 288"
          ...
          <div className="max-w-5xl mx-auto px-4 md:px-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm -mt-8 md:-mt-12 relative z-10 p-6 md:p-8">
              <AboutClientSection
                name={name} headline={headline} profileImage={profileImg}
                skills={skills} stats={stats} bio={bio}
              />
            ...
            {/* ── Social Profile Cards (enriched: bio, following, external link) ── */}
            {profileCards.length > 0 && (
              <div className="mt-6 bg-white rounded-2xl shadow-sm p-6 md:p-8">
                ...
                <SocialProfileCardsGrid accounts={profileCards} />
              </div>
            )}
            ...
            <SectionsAnimator>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4"
                data-stagger="0.10" data-stagger-preset="scale">
                {stats.map(s => (
              ...
              {skills.length > 0 && (
                <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8 mt-4" data-reveal="fadeUp">
                  <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
                    <Zap className="w-5 h-5 text-primary" />Skills &amp; Specialties
                  </h2>
                  <div className="flex flex-wrap gap-2">
                    {skills.map(s => (
                      <span key={s} className="px-4 py-1.5 bg-accent-subtle text-sm font-medium
                        text-foreground/80 rounded-full border border-border
              ...
              {experience.length > 0 && (
                <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8 mt-4" data-reveal="fadeBlur">
                  <h2 className="font-bold text-lg mb-6 flex items-center gap-2">
                    <Star className="w-5 h-5 text-primary" />Experience
                  </h2>
                    ...
                    <div className="space-y-6">
                      {experience.map(exp => (
                        <div key={exp.id} className="flex gap-5 relative">
                          <div className="flex-shrink-0 w-10 h-10 rounded-full bg-primary/10 border-2
                            border-primary/40 flex items-center justify-center z-10">
              ...
              {achievements.length > 0 && (
                <div className="bg-white rounded-2xl shadow-sm p-6 md:p-8 mt-4" data-reveal="slideLeft">
                  <h2 className="font-bold text-lg mb-5 flex items-center gap-2">
                    <Award className="w-5 h-5 text-primary" />Achievements &amp; Recognition
                  </h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {achievements.map(a => (
                      <div key={a.id} className="p-4 rounded-xl border border-border bg-accent-subtle/30
                        hover:border-primary/40 transition-colors">
                        <div className="flex items-center justify-between mb-1.5">
                          <h4 className="font-semibold text-sm">{a.title}</h4>
                          <span className="text-xs text-primary font-bold">{a.year}</span>
                        </div>
                        {a.description && (
              ...
              {/* Commission CTA */}
              <div className="mt-4 mb-10 rounded-2xl p-8 text-center glass border border-white/60
                shadow-sm" data-reveal="scale">
                <h3 className="text-2xl font-extrabold mb-2">Want a custom piece?</h3>
                <p className="text-muted-foreground mb-6 max-w-md mx-auto leading-relaxed">
                  Commission original artwork tailored to your vision. Response within 24 hours.
                </p>
                <Link href="/commission" className="btn-base inline-flex items-center gap-2 px-8 py-3.5
                  bg-primary text-white rounded-full font-semibold hover:bg-primary-light shadow-md

...
import { getBlogPosts } from '@/lib/appwrite-server';
                  ...
                  <div className="card-base overflow-hidden hover:shadow-2xl transition-all duration-300 md:flex">
                    {featured.coverImage && (
                      <div className="relative w-full md:w-1/2 aspect-video md:aspect-auto md:min-h-[320px] bg-accent-subtle overflow-hidden">
                        <Image src={String(featured.coverImage)} alt={String(featured.title)} fill
                    ...
                    <div className="p-8 md:w-1/2 flex flex-col justify-center">
                      ...
                      {featured.category && <p className="text-sm text-muted-foreground mb-2 capitalize">{String(featured.category)}</p>}
                      <h2 className="text-2xl md:text-3xl font-extrabold mb-4 group-hover:text-primary transition-colors">{String(featured.title)}</h2>
                      {featured.excerpt && <p className="text-muted-foreground leading-relaxed mb-6 line-clamp-3">{String(featured.excerpt)}</p>}
                      <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap">
                        <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5" />{String(featured.author ?? 'SR Arts')}</span>
                        <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" />
                          {new Date(String(featured.$createdAt)).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <Link key={post.$id} href={`/blog/${post.$id}`} className="group">
                      <div className="card-base overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 h-full flex flex-col">
                        ...
                        <div className="p-5 flex flex-col flex-1">
                          ...
                          <h3 className="font-bold text-lg mb-2 group-hover:text-primary transition-colors line-clamp-2 flex-1">{String(post.title)}</h3>
                          {post.excerpt && <p className="text-sm text-muted-foreground line-clamp-2 mb-4">{String(post.excerpt)}</p>}

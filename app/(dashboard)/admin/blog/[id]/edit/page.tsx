...
export default function EditBlogPage() {
  ...
  const [prevStatus, setPrevStatus] = useState<string>('draft');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (justPublished && BASE_URL) {
        void fetch('/api/indexing', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ urls: [`${BASE_URL}/blog/${form.slug || id}`] }),
        });
      }
    ...
    <div className="min-h-screen bg-[#f7f8fa]">
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-border p-5 shadow-sm space-y-4">
          ...
          {form.status === 'published' && prevStatus !== 'published' && (
            <p className="text-xs text-green-600 font-medium">
              ✓ Saving will publish & submit to search engines.
            </p>
          )}

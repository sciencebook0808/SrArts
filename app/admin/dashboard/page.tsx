...
import {
  LayoutDashboard, ImageIcon, FolderOpen, FileText, ShoppingCart,
  LogOut, Plus, Edit, Trash2, Search, Star, StarOff, Check, ChevronDown,
  User, Menu, BarChart2, RefreshCw, ExternalLink, Loader2,
...
type TabType = 'overview' | 'artworks' | 'categories' | 'blog' | 'commissions' | 'profile';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={() => void load()} className="p-2.5 rounded-xl border border-border bg-white hover:bg-accent-subtle transition-colors" title="Refresh">
              ...
              <tbody className="divide-y divide-border">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-accent-subtle/20 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground font-mono hidden md:table-cell truncate max-w-[120px]">
                      {item.slug}
                    </td>
                    <td className="px-4 py-3">
            ...
            <div key={item.id} className="bg-white rounded-2xl border border-border p-4 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {item.projectTitle && (
                    <p className="text-sm mt-1 font-medium text-foreground/80">{item.projectTitle}</p>
                  )}
                  {item.description && (
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-border p-5 shadow-sm space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-sm font-medium">Headline <span className="text-muted-foreground font-normal text-xs">(like LinkedIn tagline)</span></label>
        ...
        <div className="space-y-1">
          <label className="text-sm font-medium">Skills <span className="text-muted-foreground font-normal text-xs">(comma-separated)</span></label>
      ...
      {/* Stats */}
      <div className="bg-white rounded-2xl border border-border p-5 shadow-sm space-y-4">
        <h2 className="font-bold text-sm">Display Stats <span className="text-muted-foreground font-normal text-xs">(shown on About page)</span></h2>

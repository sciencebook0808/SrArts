...
import {
  LayoutDashboard, Image, FolderOpen, FileText,
  ShoppingCart, LogOut, Plus, Edit, Trash2, Search,
  Eye, EyeOff, Star, StarOff, X, Check, ChevronDown,
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border capitalize ${map[status] ?? 'bg-gray-100 text-gray-700 border-gray-200'}`}>
      {status}
    </span>
  );
}
...
// ─── Overview tab ──────────────────────────────────────────────────────────────
function OverviewTab({ stats }: { stats: { artworksTotal: number; blogTotal: number; ordersTotal: number } }) {
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <StatCard label="Total Artworks"   value={stats.artworksTotal} icon={Image}       color="bg-violet-100 text-violet-600" />
...
function ArtworksTab() {
  ...
  const toggle = async (item: Record<string, unknown>, field: 'status' | 'featured') => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={load} className="p-2.5 rounded-xl border border-border bg-white hover:bg-accent-subtle transition-colors" title="Refresh">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-border p-12 text-center">
          <Image className="w-10 h-10 text-muted-foreground mx-auto mb-3 opacity-30" />
          <p className="text-muted-foreground text-sm">No artworks yet. <Link href="/admin/artworks/new" className="text-primary underline">Add your first artwork</Link>.</p>
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-3">
                        {item.imageUrl ? (
                          <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-accent-subtle">
                            <NextImage src={String(item.imageUrl)} alt="" width={40} height={40} className="object-cover w-full h-full" />
                          </div>
                        ) : (
                        ...
                        <span className="font-medium truncate max-w-[140px]">{String(item.title ?? '–')}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell capitalize">{String(item.category ?? '–')}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => toggle(item, 'status')} title="Toggle status">
                        <StatusBadge status={String(item.status ?? 'draft')} />
                      </button>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <button onClick={() => toggle(item, 'featured')} className="text-muted-foreground hover:text-yellow-500 transition-colors">
                        {item.featured
                          ? <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                          : <StarOff className="w-4 h-4" />}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground hidden lg:table-cell">{String(item.views ?? 0)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        <Link href={`/admin/artworks/${item.$id}/edit`}
                          className="p-1.5 rounded-lg hover:bg-primary/10 text-primary transition-colors">
                          <Edit className="w-4 h-4" />
                        </Link>
                        <button onClick={() => setDelId(String(item.$id))}
  ...
  const add = async () => {
    ...
    try {
      await fetch('/api/categories', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: name.trim(), slug: name.trim().toLowerCase().replace(/\s+/g, '-'), order: cats.length }),
      });
      ...
      <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm">
        {loading ? (
          ...
          <ul className="divide-y divide-border">
            {cats.map(cat => (
              <li key={String(cat.$id)} className="flex items-center justify-between px-4 py-3 hover:bg-accent-subtle/20 transition-colors">
                <div>
                  <p className="font-medium text-sm">{String(cat.name)}</p>
                  <p className="text-xs text-muted-foreground">/{String(cat.slug)}</p>
                </div>
                <button onClick={() => setDelId(String(cat.$id))}
...
function BlogTab() {
  ...
  const toggleStatus = async (post: Record<string, unknown>) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={load} className="p-2.5 rounded-xl border border-border bg-white hover:bg-accent-subtle transition-colors">
          <RefreshCw className="w-4 h-4" />
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-border p-12 text-center">
          ...
          <p className="text-muted-foreground text-sm">No blog posts yet. <Link href="/admin/blog/new" className="text-primary underline">Write your first post</Link>.</p>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border border-border overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
                  ...
                  <tr key={String(post.$id)} className="hover:bg-accent-subtle/20 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        {post.coverImage ? (
                          <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 bg-accent-subtle">
                            <NextImage src={String(post.coverImage)} alt="" width={40} height={40} className="object-cover w-full h-full" />
                          </div>
                        ) : (
                          <div className="w-10 h-10 rounded-lg bg-accent-subtle shrink-0 flex items-center justify-center">
                        ...
                        <span className="font-medium truncate max-w-[160px]">{String(post.title ?? '–')}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell">{String(post.author ?? 'SR Arts')}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => toggleStatus(post)} title="Toggle status">
                        <StatusBadge status={String(post.status ?? 'draft')} />
                      </button>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">{String(post.views ?? 0)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        ...
                        <button onClick={() => setDelId(String(post.$id))}
                          className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-colors">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
    ...
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap">
        {['all', ...STATUSES].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-colors ${filter === s ? 'bg-primary text-white' : 'bg-white border border-border text-foreground/70 hover:bg-accent-subtle'}`}>
            {s === 'all' ? 'All' : s}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(item => (
            <div key={String(item.$id)} className="bg-white rounded-2xl border border-border p-4 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-semibold">{String(item.userName ?? item.name ?? '—')}</p>
                    <StatusBadge status={String(item.status ?? 'pending')} />
                  </div>
                  <p className="text-sm text-muted-foreground mt-0.5">{String(item.userEmail ?? item.email ?? '—')}</p>
                  {item.projectTitle && (
                    <p className="text-sm mt-1 font-medium text-foreground/80">{String(item.projectTitle)}</p>
                  )}
                  {item.description && (
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{String(item.description)}</p>
                  )}
                ...
                <div className="relative shrink-0">
                  <button
                    onClick={() => setOpenDropdown(openDropdown === String(item.$id) ? null : String(item.$id))}
                  ...
                  <AnimatePresence>
                    {openDropdown === String(item.$id) && (
                      ...
                      >
                        {STATUSES.map(s => (
                          <button key={s} onClick={() => updateStatus(String(item.$id), s)}
                            className={`w-full flex items-center gap-2 px-3 py-2 text-xs font-medium capitalize hover:bg-accent-subtle transition-colors text-left ${item.status === s ? 'text-primary' : 'text-foreground/80'}`}>

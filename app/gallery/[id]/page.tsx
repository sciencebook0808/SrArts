...
export default async function ArtworkPage({ params }: Props) {
  ...
  incrementArtworkViews(id, Number(artwork.views ?? 0));
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
            ...
            <div className="relative w-full aspect-square rounded-2xl overflow-hidden bg-accent-subtle shadow-lg">
              {artwork.imageUrl ? (
                <Image src={String(artwork.imageUrl)} alt={String(artwork.title)} fill
            ...
            <div className="space-y-6">
              {artwork.category && (
                <span className="text-xs font-bold text-primary uppercase tracking-widest">{String(artwork.category)}</span>
              )}
              <h1 className="text-3xl md:text-4xl font-extrabold leading-tight">{String(artwork.title)}</h1>

              {artwork.description && (
                <p className="text-muted-foreground leading-relaxed">{String(artwork.description)}</p>
              )}

              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span className="flex items-center gap-1.5"><Eye className="w-4 h-4" /> {String(artwork.views ?? 0)} views</span>
                {artwork.price && (
                  <span className="text-primary font-bold text-lg">${Number(artwork.price).toLocaleString()}</span>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4">
                ...
                {artwork.instagramLink && (
                  <a href={String(artwork.instagramLink)} target="_blank" rel="noopener noreferrer"
